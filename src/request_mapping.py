...
def map_request_to_esh_request(body: any):
    esh_request = EshRequest()
    if 'parameters' in body:
        esh_request.parameters = list(map(lambda i: Parameter.parse_obj(i), body['parameters']))
    if 'query' in body:
        esh_request.query = EshObject.parse_obj(body['query'])
    if 'rules' in body:
        esh_request.rules = list(map(lambda i: Rule.parse_obj(i), body['rules']))
    if 'configurations' in body:
        ...
        for configuration in body['configurations']:
            ...
            for k, v in configuration.items():
                if k.startswith('@'):
                    if esh_configuration.annotations is None:
                        esh_configuration.annotations = []
                        ...
                        for ek, ev in element.items():
                            if ek == 'ref':
                                ...
                                if not esh_element.annotations:
                                    esh_element.annotations = []
                                esh_element.annotations.append(AnnotationKeyValue(key=ek, value=ev))
                            else:
                                raise Exception('non expecting key: ' + ek)
